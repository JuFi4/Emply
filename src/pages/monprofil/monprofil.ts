...
export class MonprofilPage {
   ...
   /* this.user.id = 4;
    ...
    serviceUsers.getUser(this.user.id, this.user.token).subscribe(user => {
     ...
     /* TODO VANESSA : : traiter les données du profil :
      -1) Remplacer les données de test dans l'api getProfil par les variables isssues de la connexion :userId et token (voir avec Juliana où elles sont et comment les récuperer)
  ...
  enregistrer(){
   ...
   // this.serviceUsers.setUser(this.user.id, this.user.token, this.user.nom, this.user.prenom, this.user.dateNaissance, this.user.adresse,this.user.suppAdresse, this.user.codePostal, this.user.ville, this.user.telFix, this.user.telMobile)
    console.log(this.user.id, this.user.token, this.user.nom, this.user.prenom, this.user.dateNaissance, this.user.adresse, this.user.suppAdresse, this.user.codePostal, this.user.ville, this.user.telFix, this.user.telMobile)

     /* TODO VANESSA : : traiter la modification du profil :     -
      -1) Remplacer les données de test dans l'api setProfil par les variables isssues de la connexion :userId et token (voir avec Juliana où elles sont et comment les récuperer)
              -> les données du profil issues du formulaire de modification ont déja été mises, donc t'as pas à t'en occuper
      -2) Traiter le résultat de l'API : 
  ...
  doChangementMail() {
   ...
   modifierMotDePasse(){   
     ...
     - 1)  Mettre le code à la bonne place : garder la fonction modifierMotDePasse() et la completer (paramètres), ou créer une autre -> Je l'ai fait juste pour mettre le code
      ...
      -2) Remplacer les données de test dans l'api setProfil par les variables isssues de la connexion :userId et token (voir avec Juliana où elles sont et comment les récuperer)
        + avec les données du profil issues du formulaire de modification de mot de passe
  ...
  modifierEmail(){   
     ...
     - 1)  Mettre le code à la bonne place : garder la fonction modifierMotDePasse() et la completer (paramètres), ou créer une autre -> Je l'ai fait juste pour mettre le code
          d'exemple à un endroit, mais tu en fait ce que tu veux
      -2) Remplacer les données de test dans l'api setEmail par les variables isssues de la connexion :userId et token (voir avec Juliana où elles sont et comment les récuperer)
        + avec les données du profil issues du formulaire de modification de l'eamil
      -3) Traiter le résultat de l'API : 
          - Si la modification est bien enregsitrée (le "if(data)"" dans le code) : trouver comment traiter cela : alerte pour dire que c'est OK, 
            ou rien du tout et juste remettre les champs en "nom modifiable".... à toi de voir ce que tu veux mettre
          - sinon (le "else" dans le code), trouver comment traiter cela : message d'erreur "modification impossible",
